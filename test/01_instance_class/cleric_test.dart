...
void main() {
  final int fixedHp = Random().nextInt(51); // 0부터 50까지의 랜덤한 HP
  final int fixedMp = Random().nextInt(11); // 0부터 10까지의 랜덤한 MP
  final int prayerTime = Random().nextInt(60) + 1; // 1부터 60까지의 랜덤한 기도 시간

  test('pray()을 하면 mp를 회복함. 회복량은 기도한 시간(초)에 0 ~ 2포인트의 보정을 한 양. 최대 MP와 같거나 이하로 회복', () {
    ...
    expect(prevMp <= cleric.mp && realRecoveredMp <= cleric.maxMp - prevMp, isTrue);
  });
